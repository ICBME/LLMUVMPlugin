...
CAMPAIGN_TRACE_ROLLUP_KIND = "harness_optimization.campaign_trace_rollup"
...
class CampaignTraceRollupBuilder:
    ...
    kind: str = CAMPAIGN_TRACE_ROLLUP_KIND
...
def campaign_trace_rollup_path(
    evaluation_path: Path,
    *,
    suffix: str = "campaign_rollup",
) -> Path:
    return evaluation_path.with_name(f"{evaluation_path.stem}_{suffix}.json")
            ...
            values.append(
                {
                    ...
                    "value": value,
                    "delta_from_previous": mapping(summary.get("coverage_delta")).get(
                        name
                    ),
                }
            )
