...
from connector_observe.schema import normalize_artifact_refs

from .topology import ConnectorEdge, FULL_FUZZ_TOPOLOGY, PipelineTopology
...
class PipelineOrchestrator:
    ...
    def _validate_step(self, step: StepSpec) -> ConnectorEdge:
        try:
            ...
            raise ValueError(f"unknown connector in step {step.name!r}: {step.connector}") from exc
    ...
    def write_topology(self) -> None:
        if self.topology_out is None:
            return
        self.topology_out.parent.mkdir(parents=True, exist_ok=True)
        self.topology_out.write_text(
            json.dumps(self.topology.to_json(), indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
...
def external_command_step(
    *,
    ...
    command_timeout_s = policy.timeout_s
    step_policy = StepPolicy(
        fail_open_observation=policy.fail_open_observation,
        fail_main_on_step_error=policy.fail_main_on_step_error,
    )
    return StepSpec(
        name=name,
        connector=connector,
        handler=lambda _context: subprocess.run(
            command,
...
def validate_topology(topology: PipelineTopology) -> dict[str, ConnectorEdge]:
    ...
    for edge in topology.connectors:
        existing = edges.get(edge.name)
        if existing is not None and (
            existing.from_layer != edge.from_layer or existing.to_layer != edge.to_layer
        ):
            raise ValueError(
                f"connector {edge.name!r} has inconsistent topology edges: "
                f"{existing.from_layer}->{existing.to_layer} vs {edge.from_layer}->{edge.to_layer}"
            )
        edges.setdefault(edge.name, edge)
    return edges
